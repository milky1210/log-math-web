...
test.describe('学習モード', () => {
  ...
  test('章の一覧が表示される', async ({ page }) => {
    // 章のボタンが表示される（ボタン内のテキストを検索）
    await expect(page.getByRole('button', { name: /Log算の基礎/ })).toBeVisible();
    await expect(page.getByRole('button', { name: /時間の感覚/ })).toBeVisible();
  });

  test('章を選択するとスライドが表示される', async ({ page }) => {
    // Log算の基礎を選択
    await page.locator('button:has-text("Log算の基礎")').click();
    await page.waitForTimeout(300);
  ...
  test('スライドを進める・戻れる', async ({ page }) => {
    ...
    // 2枚目のスライドが表示される
    await expect(page.getByText('掛け算は足し算に')).toBeVisible();
